...
/*****************************************************************************/
public class Example22_PanelInfo extends Examples_Panel
	implements ActionListener
{
	...
	public Example22_PanelInfo(Example22_I_Callback callback)
	{
		...
		Example22_SectionManagement.sectionSetIconAdmin(butConfig);
	...
	public void actionPerformed(ActionEvent evt)
	{
		...
		if(obj==butConfig && currNameSvc != null)
			Example22_SectionManagement.openDialogChange(currNameSvc);
	}
	...
	public void setInfo(String nameSvc)
	{
		...
		setTextArea(txtDescr, Example22_UtilsSVC.scmGetDescription(nameSvc));
	...
	void setInfoConfig(String nameSvc)
	{
		...
		lblNameLong.setText(arrConfig==null ? "" : arrConfig[Example22_UtilsSVC.NDX_CONFIG_NAMELONG]);

		String login = (arrConfig==null ? "" : arrConfig[Example22_UtilsSVC.NDX_CONFIG_LOGIN]);
		setLabelVal(lblLabelLogin, lblLogin, login, (login != null && !login.isEmpty()));

		setTextArea(txtPathname, (arrConfig==null ? "" : arrConfig[Example22_UtilsSVC.NDX_CONFIG_PATHNAME]));

		String type = (arrConfig==null ? "" : arrConfig[Example22_UtilsSVC.NDX_CONFIG_TYPE]);
		lblType.setText(Example22_UtilsSVC.toStringServiceType(type));

		String start = (arrConfig==null ? "" : arrConfig[Example22_UtilsSVC.NDX_CONFIG_START]);
		lblStart.setText(Example22_UtilsSVC.toStringStartType(start,Example22_UtilsSVC.scmIsDelayedAutoStart(nameSvc)));
	...
	void setInfoStatus(String nameSvc)
	{
		...
		int pid = Example22_UtilsSVC.toInt(arrStatus==null ? "" : arrStatus[Example22_UtilsSVC.NDX_STATUS_PRCSSID]);
		setLabelVal(lblLabelPrcssID, lblPrcssID, ""+pid, (pid > 0));

		String status = (arrStatus==null ? "" : arrStatus[Example22_UtilsSVC.NDX_STATUS_CURRENT]);
		lblStatus.setText(Example22_UtilsSVC.toStringCurrentStatus(status));
	}

	/*******************************************/
	void setInfoDepend(String[] arrConfig)
	{
		String depend = (arrConfig==null ? null : arrConfig[Example22_UtilsSVC.NDX_CONFIG_DEPENDS]);
		...
		if(arrDpnd != null)
			for(int i=0;i < arrDpnd.length;i++)
			{
				...
				if(!s.isEmpty())
					addVectNameSvc(vect, s, Example22_UtilsSVC.scmGetNameLong(s));
			}
	...
	{
		String[][] arrRef = Example22_UtilsSVC.scmEnumDependentServices(nameSvc, Example22_UtilsSVC.SERVICE_STATE_ALL, false);
		Vector<String> vect = new Vector<String>();
		...
		if(arrRef != null)
			for(int i=0;i < arrRef.length;i++)
			{
				...
				if(arr != null && arr.length > 0)
					addVectNameSvc(vect, arr[0], (arr.length > 1 ? arr[1] : null));
			}
